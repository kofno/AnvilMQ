...
- **Idempotent, non-destructive upload.** The sidecar runs a poll loop that federated-logs in to Azure and runs `az storage blob upload-batch --account-name <account> --destination <container>[/<prefix>] --source /data/backups --pattern 'snapshot-*.db' --auth-mode login --overwrite false` every `backup.upload.pollSeconds` (default 30). Snapshots are immutable, uniquely timestamped files, so `--overwrite false` means each is uploaded exactly once and never re-sent, and nothing ever deletes remote blobs when the broker's local retain-K reaps a local file.
- **Workload Identity, no secrets.** The sidecar authenticates with the webhook-injected `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_FEDERATED_TOKEN_FILE`, and `AZURE_AUTHORITY_HOST` — it federated-logs in via `az login --service-principal --federated-token`, so no connection strings or account keys are stored anywhere. The chart labels the pod `azure.workload.identity/use: "true"` and creates a ServiceAccount annotated with `azure.workload.identity/client-id`.
- **Strict, read-only root.** The sidecar keeps the same hardened `securityContext` as the broker (non-root, `readOnlyRootFilesystem: true`, all capabilities dropped, no privilege escalation). A small writable `cli-scratch` emptyDir is mounted at `/azure` and used for `AZURE_CONFIG_DIR` (the Azure CLI's profile/logs) so the root filesystem can stay read-only.
- **Swappable image.** `backup.upload.image` (default `mcr.microsoft.com/azure-cli`) is configurable; point it at any image that bundles the Azure CLI (`az`) — e.g. a pinned, mirrored, or hardened build.
